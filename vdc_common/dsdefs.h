...
//
//  Copyright (c) 2012-2023 plan44.ch / Lukas Zeller, Zurich, Switzerland
//
...
typedef enum {
  ...
  STOP_S = 15,                                    ///< stop
  RESERVED_WILDCARD = 16,                         ///< reserved in DS, might be used in special cases in p44vdc internally
...
typedef enum {
  ...
  ct_progress = 128, ///< extra progress event reporting button state, when no other event applies - not sent to dS, only bridges
...
typedef enum {
  ...
  sensorFunc_app = buttonFunc_app, ///< application specific sensor/dial
} VdcSensorFunc;
