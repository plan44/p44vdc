  ...
  mGeolocation() // default location will be set from timeutils
{
  ...
  assert(gSharedVdcHostP==nullptr); // must not already exist!
  gSharedVdcHostP = this;
...
{
  return VdcHostPtr(gSharedVdcHostP);
}
...
SceneNo VdcHost::getSceneIdByKind(string aSceneKindName)
{
  ...
  int sceneNo;
  if (uequals(aSceneKindName.c_str(), "#", 1)) aSceneKindName.erase(0,1); // allow #n specification as generated by sceneText(x,x,true)
...
string VdcHost::sceneText(SceneNo aSceneNo, bool aIsGlobal, bool aAsId)
{
