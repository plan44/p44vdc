...
// MARK: - async interface for ds485 operations

void Ds485Comm::scanDs485Bus(StatusCB aStatusCB)
{
  mDs485ClientThread->executeOnChildThreadAsync(boost::bind(&Ds485Comm::scanDs485BusSync, this, _1), aStatusCB);
}
...
ErrorPtr Ds485Comm::scanDs485BusSync(ChildThreadWrapper &aThread)
{
...
void Ds485Comm::ds485ClientThread(ChildThreadWrapper &aThread)
{
  ...
  while(!aThread.shouldTerminate()) {
    ...
    OLOG(LOG_WARNING, "ds485_client_open2 failed, retrying in %lld seconds", DS485_THREAD_RESTART_INTERVAL/Second);
    MainLoop::sleep(DS485_THREAD_RESTART_INTERVAL);
  }
